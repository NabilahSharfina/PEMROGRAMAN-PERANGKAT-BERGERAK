...
import com.nabilah_19104025.modul9.databinding.ActivitySettingPreferenceBinding
...
class SettingPreferenceActivity : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onClick(p0: View?) {
        if (p0?.id == R.id.btn_save) {
            ...
            val land = binding.edtLand.text.toString().trim()
            val job = binding.edtJob.text.toString().trim()
            ...
            if (land.isEmpty()) {
                binding.edtLand.error = getString(R.string.field_required)
                return
            }
            if (job.isEmpty()) {
                binding.edtJob.error = getString(R.string.field_required)
                return
            }
            ...
            saveSetting(name, email, age, phoneNo, land, job, isLoveMU)
    ...
    private fun showPreferenceInForm() {
        ...
        binding.edtLand.setText(settingModel.land)
        binding.edtJob.setText(settingModel.job)
    }

    private fun saveSetting(name: String, email: String, age: String, phoneNo: String, land: String, job: String, isDark: Boolean) {
        ...
        settingModel.land = land
        settingModel.job = job
